...
class SmartHomeGenerator extends AbstractGenerator {
	...
	class «room.name.toFirstUpper»«IF parentRoom !==null»(«parentRoom.name»)«ENDIF»:
	    ...
	    class «dev.name.toFirstUpper»«IF parentDevice !==null»(«parentDevice.name»)«ENDIF»:
	        ...
	        «ELSE»def __init__(self, «IF parentDeviceProperties.size > 0»«printPropertiesName(parentDeviceProperties)»«IF dev.properties.size > 0», «ENDIF»«ENDIF»«printPropertiesName(dev.properties)»):
	            «IF parentDeviceProperties.size > 0 && propertiesHasExpression(parentDeviceProperties, dev.exp) == false»super().__init__(«printPropertiesName(parentDeviceProperties)»)«ENDIF»
	            «IF parentDeviceProperties.size > 0 && propertiesHasExpression(parentDeviceProperties, dev.exp) == true»
	            if(«getPropertyConstraintForSuperClass(parentDeviceProperties, dev.exp)»):
	                super().__init__(«printPropertiesName(parentDeviceProperties)»)
	            else:
	                raise ValueError("Invalid value for parent class properties")
	            super().__init__(«printPropertiesName(parentDeviceProperties)»)«ENDIF»
	            ...
	            «ENDIF»
	            «ENDFOR»
        «printGetSetPrivateProperties(dev.properties)»
	        «ENDIF»
	...
	def boolean propertiesHasExpression(List<Property> properties, EList<Expression> expList){
		var hasExpression = false
		
		hasExpression = properties.filter[t | hasPropertyConstraint(expList, t.name) === true].size > 0
		
		return hasExpression
	}
	
	def String getPropertyConstraintForSuperClass(List<Property> properties, EList<Expression> expList){
		val arrayList = newArrayList
		for(pro: properties){
			val tempConstraint = getPropertyConstraint(expList, pro.name)
			if(tempConstraint !== null){
					arrayList.add(tempConstraint.complieExpByDispatch)
			}
		}
